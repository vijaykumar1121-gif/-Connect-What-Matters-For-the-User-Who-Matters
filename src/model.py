...
class HierarchicalAttention(nn.Module):
    """Hierarchical attention mechanism for document structure understanding."""
    ...
    def forward(self, embeddings: torch.Tensor, level: str = 'sentence') -> torch.Tensor:
        """Apply hierarchical attention based on level."""
        if level == 'sentence':
...
class FewShotLearner(nn.Module):
    """Few-shot learning module for rapid adaptation to new personas/domains."""
...
class PersonaDocumentModel(nn.Module):
    """
    Enhanced custom NLP model for persona-driven document analysis.
    Features: Few-shot learning, hierarchical attention, contrastive learning.
    """
    ...
    def __init__(self, model_name: str = "sentence-transformers/all-MiniLM-L6-v2", 
        ...
        # Pre-trained transformer for text encoding
        ...
        # Custom layers for persona-document alignment
        ...
        # Attention mechanism for persona-document interaction
        self.attention = nn.MultiheadAttention(hidden_size, num_heads=8, batch_first=True)
        
        # Enhanced classification head
        self.classifier = nn.Sequential(
            nn.Linear(hidden_size * 4, hidden_size),  # Increased input size
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(hidden_size, hidden_size // 2),
            nn.ReLU(),
        ...
        # Enhanced explanation generation head
        self.explanation_head = nn.Sequential(
            nn.Linear(hidden_size * 4, hidden_size),
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(hidden_size, 7)  # Increased explanation categories
        )
    
    def encode_text(self, text: str) -> torch.Tensor:
        """Encode text using the pre-trained transformer."""
    ...
    def forward(self, persona_text: str, job_text: str, document_text: str,
        ...
        """
        Enhanced forward pass with few-shot and contrastive learning.
        
        Args:
            persona_text: Persona description
            job_text: Job-to-be-done description
            document_text: Document section/subsection text
            support_examples: Few-shot support examples
            contrastive_mode: Whether to use contrastive learning
            
        Returns:
            Dictionary containing relevance score, explanation features, and additional outputs
        """
        # Encode inputs
        ...
        # Apply hierarchical attention to document
        ...
        # Apply attention between persona-job and document
        ...
        # Few-shot learning if support examples provided
        ...
        # Concatenate features (including few-shot score)
        ...
        if contrastive_mode and support_examples:
            positive_emb = support_embs[0]  # First support example as positive
            negative_emb = self.encode_text("This is a completely unrelated text.")  # Negative example
    ...
    def predict(self, persona_text: str, job_text: str, document_text: str,
        ...
        """
        Enhanced prediction with few-shot learning.
    ...
    def _generate_enhanced_explanation(self, explanation_features: torch.Tensor, few_shot_score: float) -> str:
        """Generate enhanced explanation with few-shot information."""
        explanations = []
